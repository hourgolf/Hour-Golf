...
export default async function handler(req, res) {
  ...
  try {
    // 1) Lookup member
    const members = await supaFetch(key, "members", `?email=eq.${encodeURIComponent(cleanEmail)}`);
    if (!members.length) {
      return res.status(404).json({ error: "No member found with that email" });
    }
    const member = members[0];

    // 2) Load tier config (might be null, tolerate it)
    let tierCfg = null;
    if (member.tier) {
      try {
        const rows = await supaFetch(key, "tier_config", `?tier=eq.${encodeURIComponent(member.tier)}`);
        tierCfg = rows[0] || null;
      } catch (_) { /* ignore */ }
    }
    ...
    try {
      upcoming = await supaFetch(
        key,
        "bookings",
        `?customer_email=eq.${encodeURIComponent(cleanEmail)}&booking_status=eq.Confirmed&booking_start=gte.${now.toISOString()}&order=booking_start.asc&limit=20`
      );
    } catch (_) { upcoming = []; }
    ...
    try {
      monthBookings = await supaFetch(
        key,
        "bookings",
        `?customer_email=eq.${encodeURIComponent(cleanEmail)}&booking_status=eq.Confirmed&booking_start=gte.${monthStart}&booking_start=lt.${monthEnd}&order=booking_start.asc`
      );
