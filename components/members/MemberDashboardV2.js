...
// Prototype dashboard. Swaps in for /members/dashboard at /members/dashboard-v2
// so the original stays untouched while we evaluate. Behavior changes vs. v1:
//   1) Three redundant usage cards collapsed to one progress bar.
//   2) Promotes the next booking into a hero with countdown + access-code line.
//   3) QR moves out of the stat row into a single icon button next to the
//      tier badge, and the QR image is only fetched on open (was lazy-loaded
//      thumb on every dashboard render — eager external API call leaked the
//      member id to qrserver.com on every page view).
//   4) "Contact us to cancel" becomes a tappable mailto/tel link using the
//      existing branding.support_email / support_phone fields.
//   5) Status colors switched to CSS vars so tenants can theme them; we
//      define safe fallbacks inline so the prototype still renders cleanly
//      on any tenant whose theme hasn't shipped overrides yet.
...
export default function MemberDashboardV2({ member, tierConfig, refresh, showToast }) {
  ...
  // Tick the clock once a minute so the next-booking countdown stays live.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      ...
      if (params.get("purchased")) {
        ...
        window.history.replaceState({}, "", "/members/dashboard-v2");
      }
    }
  }, []);
  ...
  // Tier color for the inline pill (still pulls from TIER_COLORS — flagged
  // as a tenant gap in the audit).
  ...
  // Hero: the next upcoming booking (closest start in the future).
  ...
  return (
    <>
      {/* Compact greeting strip — name + tier + QR shortcut on one row,
          dropping the standalone H1 from v1 that consumed ~70px on mobile
          for no signal. */}
      ...
      {/* Hero — Next booking. Hidden when the member has nothing upcoming so
          they get a clear "Book a bay" call-to-action instead. */}
      {nextBooking ? (
        (() => {
      ...
      {/* Usage — single progress bar replaces the three redundant cards
          (Used / Remaining / Allowance). Bonus + overage + shop credit
          stay as small chips below so the at-a-glance numbers still surface
      ...
      <div className="mem-section mem2-usage">
        ...
        {/* Chip row: bonus / overage / shop credit. Each renders only when
            non-zero so the row collapses cleanly when nothing extra applies. */}
      ...
      {/* QR modal — same image source as v1 but only fetched on open
          (no eager thumbnail on every dashboard load). Production should
          self-host with `qrcode.react` to remove the third-party dependency
          entirely; flagged in the audit but kept as-is for the prototype. */}
      ...
      {/* Loyalty (carried over, lightly cleaned up — adds a "what is the
          reward" hint above the bars so members understand what they're
          earning before they hit the threshold). */}
      ...
      {/* Announcements (carried over from v1). */}
      {news.length > 0 && (
        <div className="mem-section">
      ...
      {/* Other upcoming bookings (everything except the hero). Hidden when
          the only upcoming booking is already in the hero. */}
      ...
      {/* Shipments — unchanged from v1, still hidden when nothing in transit. */}
      {shipments.length > 0 && (
        <div className="mem-section">
      ...
      {/* This month's activity — kept; gives members a quick "I've been here X
      ...
      {/* Footer breadcrumb so the prototype is obviously a prototype. */}
      <div style={{ marginTop: 24, padding: "12px 14px", textAlign: "center", fontSize: 11, color: "var(--text-muted)", fontFamily: "var(--font-mono)", letterSpacing: 0.6, textTransform: "uppercase" }}>
        Dashboard prototype · <a href="/members/dashboard" style={{ color: "var(--primary)", textDecoration: "underline" }}>back to current</a>
      </div>
    </>
  );
}
