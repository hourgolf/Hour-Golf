  ...
  switch (type) {
    ...
    case "payment.created": {
      ...
      const memberResp = await fetch(
        `${SUPABASE_URL}/rest/v1/members?tenant_id=eq.${tenantId}&square_customer_id=eq.${encodeURIComponent(customerId)}&select=email,name,shop_credit_balance`,
        { headers: { apikey: serviceKey, Authorization: `Bearer ${serviceKey}` } }
      );
      ...
      // If the POS auto-applied the member's gift card, Square has
      // already decremented their gift card balance. Mirror the debit
      // on our side so shop_credit_balance stays in lockstep. We
      // detect this via source_type === 'gift_card'; Square issues a
      // separate payment webhook per tender, so a mixed card+gift
      // sale fires twice and each is handled independently. Guarded
      // by result.recorded to stay idempotent with the dedup above.
      if (result?.recorded && sourceType === "gift_card") {
        try {
          const currentBalance = Number(member.shop_credit_balance || 0);
          const usedDollars = amountCents / 100;
          const newBalance = Math.max(0, Math.round((currentBalance - usedDollars) * 100) / 100);
          await fetch(
            `${SUPABASE_URL}/rest/v1/members?tenant_id=eq.${tenantId}&email=eq.${encodeURIComponent(member.email)}`,
            {
              method: "PATCH",
              headers: {
                apikey: serviceKey,
          ...
          await fetch(`${SUPABASE_URL}/rest/v1/shop_credits`, {
            method: "POST",
            headers: {
              apikey: serviceKey,
              Authorization: `Bearer ${serviceKey}`,
            ...
            body: JSON.stringify({
              ...
              reason: receiptNumber
                ? `In-store Square POS — receipt #${receiptNumber}`
                : "In-store Square POS purchase",
            }),
          });
        } catch (e) {
          console.error(`square-webhook: gift card balance sync failed for ${member.email}:`, e.message);
        }
      }
