...
// Build the absolute base URL the request landed on. Used by email
// templates so a member's "View dashboard" CTA points back to the
// tenant subdomain they actually use, not Hour Golf's by accident.
//
// Honors x-forwarded-proto / x-forwarded-host (Vercel sets both) so
// links resolve to https://hourgolf.vercel.app rather than
// http://internal-hostname for any deployment behind a proxy.
export function getRequestOrigin(req) {
  const headers = req?.headers;
  if (!headers) return null;
  const host =
    headers["x-forwarded-host"] || headers.host || null;
  if (!host) return null;
  const proto =
