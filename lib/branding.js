...
export const FALLBACK_BRANDING = {
  ...
  // Multi-tenant readiness fields (migration 20260419000000). NULL by
  // default so tenants who haven't configured these get sensible runtime
  // fallbacks (HG defaults baked into consumers — see DEFAULT_BAYS,
  // DEFAULT_CANCEL_CUTOFF_HOURS below).
  bays: null,
  bay_label_singular: null,
  cancel_cutoff_hours: null,
  facility_address: null,
  tier_colors: null,
  // Hard per-day usage ceiling for self-service extension. Distinct
  // from tier_config.included_hours (monthly). NULL = no cap.
  max_daily_hours_per_member: null,
  // Member-dashboard empty-state hero headline. NULL → falls back to
  // a generic "Ready to play?" so non-HG tenants don't inherit
  // golf-specific copy.
  dashboard_empty_headline: null,
  app_name: "Ourlee",
};

export const DEFAULT_DASHBOARD_EMPTY_HEADLINE = "Ready to play?";

// Runtime defaults for the multi-tenant readiness fields. Consumers
// should prefer the branding value when set, then fall back to these.
// Keeping them here (not in lib/constants) so anything reading from
// branding has the fallback in lockstep.
export const DEFAULT_BAYS = ["Bay 1", "Bay 2"];
export const DEFAULT_BAY_LABEL_SINGULAR = "Bay";
export const DEFAULT_CANCEL_CUTOFF_HOURS = 6;

// Resolve the active bay list + singular noun for the given branding
// object. Used by every client surface that renders bay-keyed UI
// (member booking sheet/grid, admin Today/Week/Reports/BookingForm).
// Filters non-string entries defensively in case a tenant row stores
// something malformed.
export function resolveBays(branding) {
  const fromBranding = branding?.bays;
  if (Array.isArray(fromBranding) && fromBranding.length > 0) {
    const cleaned = fromBranding.filter((b) => typeof b === "string" && b.length > 0);
    if (cleaned.length > 0) return cleaned;
  }
  return DEFAULT_BAYS;
}

export function resolveBayLabel(branding) {
  return branding?.bay_label_singular || DEFAULT_BAY_LABEL_SINGULAR;
}
...
//
// Routes the URL through /_next/image so Vercel's built-in optimizer
// transcodes to AVIF/WebP for the phone that's loading it, resizes to a
// reasonable desktop-max width (1920), and caches at the Edge. The
// Supabase-hosted background PNGs are ~3MB uncompressed; after the
// optimizer serves AVIF/WebP we're typically under 300KB — a single
// request, cached at Edge after first hit. next.config.js must whitelist
// the source hostname via images.remotePatterns (it does).
//
// If the URL is already relative or we can't safely encode it, fall back
// to the raw URL — graceful degradation, the page still works.
export function buildBackgroundImageRule(branding) {
  const b = branding || FALLBACK_BRANDING;
  if (!b.background_image_url) return "";
  const safeUrl = optimizedImageUrl(b.background_image_url, {
    width: 1920,
    quality: 70,
  });
  const safe = String(safeUrl).replace(/['"]/g, "");
  return `body {
...
// Wrap a remote image URL in /_next/image so Vercel transcodes + resizes
// it on the fly. Safe to call in both CSS contexts (body background) and
// inline <img src=...>. Width is the max render width we expect; the
// optimizer will never serve larger than that regardless of source.
//
// If the URL can't be encoded (falsy, already relative to /_next/image),
// return it unchanged — fallback to the raw image, the page still works.
export function optimizedImageUrl(src, { width = 1200, quality = 75 } = {}) {
