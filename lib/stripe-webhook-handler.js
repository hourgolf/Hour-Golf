  ...
  switch (event.type) {
    ...
    case "checkout.session.completed": {
      ...
      if (session.metadata?.type === "guest_shop") {
        ...
        // Buyer email: Stripe's hosted Checkout already sends a
        // tenant-Stripe-branded receipt when customer_email is set on
        // the session, so we don't double-send here. Admin-side
        // notification reuses the existing sendShopOrderNotification
        // helper used by member-shop.js, but enrichment of item titles
        // would require a second query — defer that polish to Phase 2.
        break;
      }
