...
import { safePush } from "./admin-push";
import {
  sendWelcomeEmail,
...
export async function handleStripeEvent({ event, stripe, tenantId, serviceKey, portalUrl }) {
  ...
  switch (event.type) {
    ...
    case "invoice.paid": {
      ...
      // Flip subscription_status back to active when a payment succeeds.
      // Covers the recovery path: past_due member updates their card,
      // Stripe's Smart Retries succeed, member is current again. Admin's
      // past-due chip clears on the next dashboard refresh.
      try {
        await fetch(
        ...
        console.warn("invoice.paid: subscription_status sync failed (non-fatal):", e.message);
      }
    ...
    case "customer.subscription.updated": {
      ...
      // Sync the subscription's current lifecycle status onto the
      // member row. Stripe's sub.status covers active / past_due /
      // unpaid / canceled / incomplete / trialing — all of which the
      // admin Customers tab now reads to color the past-due chip.
      ...
      console.log(`Subscription updated for customer ${stripeCustomerId}: tier=${newTier || "unchanged"}, status=${subStatus || "unchanged"}`);
      break;
    }
    ...
    // --- Payment failed ---
    //
    // Stripe fires this once per retry attempt (Smart Retries default:
    // ~3 attempts over ~2 weeks). Our response:
    //
    //   1. Record the attempt in `payments` with status='failed' so the
    //      admin sees it in Reports/per-member detail.
    //   2. Mark member.subscription_status='past_due' so the admin
    //      Customers tab shows the past-due chip.
    //   3. Email the member ONCE — on attempt_count === 1 — with a
    //      branded "your card was declined, here's Update Card".
    //      Subsequent retries don't re-email to avoid inbox spam;
    //      Stripe's own customer emails can cover the middle cadence
    ...
    case "invoice.payment_failed": {
      ...
      console.warn(`Payment failed for customer ${stripeCustomerId}: ${invoice.id} (attempt ${attemptCount})`);

      // Look up the member (by stripe_customer_id inside this tenant).
      let memberEmail = null;
      let memberName = null;
      try {
        ...
        const rows = memberResp.ok ? await memberResp.json() : [];
        if (rows.length) {
          memberEmail = rows[0].email;
          memberName = rows[0].name;
        }
      } catch (e) {
        console.warn("invoice.payment_failed: member lookup failed:", e.message);
      }

      // Record the failed attempt. Idempotency: we suffix the intent id
      // with the attempt number so Stripe re-firing the same event (or
      // multiple retries on the same invoice) doesn't insert duplicates.
      const failedPiId =
        `${invoice.payment_intent || `inv_${invoice.id}`}_fail${attemptCount}`;
      const billingMonth = invoice.period_start
        ? new Date(invoice.period_start * 1000).toISOString()
        : new Date().toISOString();
      try {
        await fetch(`${SUPABASE_URL}/rest/v1/payments?on_conflict=stripe_payment_intent_id`, {
          method: "POST",
          headers: {
            apikey: key, Authorization: `Bearer ${key}`,
            "Content-Type": "application/json",
          ...
          body: JSON.stringify({
            tenant_id: tenantId,
            member_email: memberEmail || `stripe_customer_${stripeCustomerId}`,
            billing_month: billingMonth,
            amount_cents: amount,
        ...
        console.warn("invoice.payment_failed: payments row insert failed:", e.message);
      }

      // Mark member past_due so admin sees the chip.
      if (memberEmail) {
        try {
          await fetch(
            `${SUPABASE_URL}/rest/v1/members?stripe_customer_id=eq.${encodeURIComponent(stripeCustomerId)}&tenant_id=eq.${tenantId}`,
          ...
          console.warn("invoice.payment_failed: subscription_status set failed:", e.message);
        }

        // Push admin PWA — only on the first attempt so we don't
        // re-fire every dunning cycle. safePush never throws.
        if (attemptCount === 1) {
          await safePush(tenantId, {
            title: "Member past due",
            body: `${memberName || memberEmail} — card charge failed${amount ? ` ($${(amount / 100).toFixed(2)})` : ""}`,
            url: "/admin?view=inbox",
            tag: `past-due-${memberEmail}`,
          });
        }
      }

      // Polite first-attempt email. Subsequent attempts stay silent —
      // if Stripe's own dunning emails are enabled in the dashboard,
      // those fill in the middle cadence.
      if (attemptCount === 1 && memberEmail) {
        try {
          ...
          console.error("invoice.payment_failed: member email failed:", e);
        }
      }
