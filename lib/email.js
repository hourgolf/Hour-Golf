...
export async function sendBookingConfirmation({
  ...
  const calendarTitle = `${venueName} — ${bay || "Bay"}`;
  const calendarLocation = branding?.support_email
    ? `${venueName}` // address would be ideal here; falls back to venue name when not configured
    : venueName;
  ...
  const bodyHtml = `
    ...
    <p style="margin: 0 0 14px 0;">Your bay is booked. Here are the details:</p>
    ${renderDetailBox({ palette: { cream: branding?.cream_color || "#EDF3E3" }, bodyHtml: detailHtml })}
    ${calendarLinks}
    ${accessBlock}
    <p style="margin: 0; font-size: 13px; color: #6B7A6F;">You can cancel from your member portal up to 6 hours before your booking.</p>
  `;
