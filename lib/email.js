...
// flow. `recipientEmail` is embedded so they see the exact address.
function cutoverSignInSteps(venueName, recipientEmail) {
  return `
    <ol style="margin: 0 0 14px 20px; padding: 0; font-size: 14px; line-height: 1.65;">
      <li>Open <strong>${escapeHtml(venueName)}</strong> on your phone (button below).</li>
      <li>Tap <strong>Sign in</strong> and use this email: <strong>${escapeHtml(recipientEmail)}</strong>. You don't have a password yet — enter anything for the password and we'll walk you through setting a real one.</li>
      <li>If we don't recognize you yet (i.e. you've only visited as a walk-in), you'll see a <strong>Create account</strong> button after trying. Tap that.</li>
    </ol>
  `;
}
...
export async function sendCutoverAnnouncement({
  ...
  const bodyHtml = `
    <p style="margin: 0 0 14px 0;">Hey ${escapeHtml(customerName || "there")},</p>
    <p style="margin: 0 0 14px 0;"><strong>Big heads-up:</strong> we're retiring the old Skedda booking page on <strong>${escapeHtml(dateStr)}</strong>. After that day, every booking happens inside the new ${escapeHtml(venueName)} app.</p>
    <p style="margin: 0 0 14px 0;">Why the change: one system means no more accidental double-bookings, your door code appears live on your phone, and you can upgrade/cancel your membership without emailing us. It's also just faster.</p>
    <p style="margin: 0 0 10px 0;"><strong>What you need to do — takes under two minutes:</strong></p>
    ${cutoverSignInSteps(venueName, to)}
    <p style="margin: 0 0 14px 0;">Do this <strong>before ${escapeHtml(dateStr)}</strong>. After that day Skedda won't accept new bookings — the only way to reserve a bay will be through the app.</p>
    <p style="margin: 0 0 12px 0; font-size: 13px; color: #6B7A6F;">Stuck on any step? Reply to this email — a human reads every response.</p>
  `;

  const { html, text } = await renderEmailLayout({
    tenantId,
    branding,
    title: `Skedda is closing on ${dateStr}`,
    preheader: `We're moving everything into the ${venueName} app. Three steps to get set up.`,
    bodyHtml,
    ctaButton: signInUrl ? { label: "Sign in now", url: signInUrl } : null,
    footerText: tenantEmail?.email_footer_text,
    footerNote: "Questions? Just reply. We'll answer fast.",
  });

  const subject = `Big change: Skedda closes ${dateStr}`;
...
export async function sendCutoverReminder({
  ...
  const bodyHtml = `
    <p style="margin: 0 0 14px 0;">Hey ${escapeHtml(customerName || "there")},</p>
    <p style="margin: 0 0 14px 0;">Quick reminder: Skedda closes in <strong>${escapeHtml(daysLabel)}</strong> (on <strong>${escapeHtml(dateStr)}</strong>), and we're not seeing a login from you in the new ${escapeHtml(venueName)} app yet.</p>
    <p style="margin: 0 0 14px 0;">After ${escapeHtml(dateStr)} you <strong>won't be able to book a bay</strong> until you've signed in — two minutes today saves you a frustrating moment the next time you want to reserve.</p>
    <p style="margin: 0 0 10px 0;"><strong>Your 3-step setup:</strong></p>
    ${cutoverSignInSteps(venueName, to)}
    <p style="margin: 0 0 14px 0; font-size: 13px; color: #6B7A6F;">If the setup is the hang-up, reply to this email or text us — we'll walk you through it in a minute flat.</p>
  `;

  const { html, text } = await renderEmailLayout({
    tenantId,
    branding,
    title: `${daysUntil === 0 ? "Today" : daysLabel + " left"}: Skedda closes ${dateStr}`,
    preheader: `You still need to sign in to the ${venueName} app before ${dateStr}.`,
    bodyHtml,
    ctaButton: signInUrl ? { label: "Sign in now", url: signInUrl } : null,
    footerText: tenantEmail?.email_footer_text,
    footerNote: "Reply here or text us — we'll help you get set up.",
  });

  const subject = daysUntil === 0
    ? `Today: Skedda is closing — sign in now`
    : `${daysLabel} left — sign in to the ${venueName} app`;
  if (_preview) return { preview: true, to, from: tenantEmail?.email_from, subject, html, text };
  return sendEmail({
    to, subject, html, text,
    fromOverride: tenantEmail?.email_from,
...
export async function sendCutoverComplete({
  ...
  const bodyHtml = alreadyOnApp
    ? `
      <p style="margin: 0 0 14px 0;">Hey ${escapeHtml(customerName || "there")},</p>
      <p style="margin: 0 0 14px 0;"><strong>Skedda is now closed.</strong> Everything lives in the ${escapeHtml(venueName)} app from today forward.</p>
      <p style="margin: 0 0 14px 0;">You're already set up — nothing changes for you. Just stop opening the Skedda page. Every future booking, door code, and receipt is in the app.</p>
      <p style="margin: 0 0 12px 0;"><strong>From now on:</strong></p>
      <ul style="margin: 0 0 14px 20px; padding: 0; font-size: 14px; line-height: 1.65;">
        <li>📅 Book bays from the app home screen</li>
        <li>🔑 Your door code appears ~10 minutes before your booking</li>
        <li>🛍️ Shop the pro shop inline</li>
        <li>⭐ Upgrade, downgrade, or cancel your membership in Account</li>
      </ul>
      <p style="margin: 0 0 12px 0; font-size: 13px; color: #6B7A6F;">Thanks for making the switch — it's a cleaner ride for both of us. Reply any time.</p>
    `
    : `
      <p style="margin: 0 0 14px 0;">Hey ${escapeHtml(customerName || "there")},</p>
      <p style="margin: 0 0 14px 0;"><strong>Skedda is now closed.</strong> All bookings live inside the ${escapeHtml(venueName)} app starting today. We haven't seen you log in yet, so here's the <strong>last-chance walkthrough</strong>:</p>
      <p style="margin: 0 0 10px 0;"><strong>Two-minute setup:</strong></p>
      ${cutoverSignInSteps(venueName, to)}
      <p style="margin: 0 0 14px 0;">Until you do, you won't be able to book a bay. Five minutes today and you're back to reserving like normal.</p>
      <p style="margin: 0 0 12px 0; font-size: 13px; color: #6B7A6F;">Reply here or text us — we'll walk you through it in a minute flat.</p>
    `;

  const { html, text } = await renderEmailLayout({
    tenantId,
    branding,
    title: `Skedda is now closed`,
    preheader: alreadyOnApp
      ? `You're set. Everything lives in the ${venueName} app from today forward.`
      : `Last-chance setup. Two minutes and you're back to booking.`,
    bodyHtml,
    ctaButton: signInUrl
      ? { label: alreadyOnApp ? `Open ${venueName}` : "Sign in now", url: signInUrl }
      : null,
    footerText: tenantEmail?.email_footer_text,
    footerNote: "See you out there.",
  });
  ...
  const subject = alreadyOnApp
    ? `Skedda is now closed — everything's in the ${venueName} app`
    : `Skedda is now closed — last-chance setup`;
  if (_preview) return { preview: true, to, from: tenantEmail?.email_from, subject, html, text };
  return sendEmail({
    to, subject, html, text,
    fromOverride: tenantEmail?.email_from,
