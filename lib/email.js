...
//
// Sent via /api/admin-broadcast-launch (admin-triggered, one-off). Links
// to /app on the tenant's own portal URL so the member lands on the
// tenant-branded install explainer. `installPath` defaults to "/app"
// but can be overridden if the explainer ever moves.
export async function sendLaunchEmail({
  tenantId, to, customerName, portalUrl, installPath = "/app",
}) {
  const { tenantEmail, branding } = await loadTenantContext(tenantId);
  const venueName = branding?.app_name || tenantEmail?.name || "Ourlee";
  const installUrl = resolvePortalUrl(portalUrl, installPath);
  ...
  const bodyHtml = `
    ...
    <p style="margin: 0 0 12px 0;"><strong>Three steps to get set up:</strong></p>
    <ol style="margin: 0 0 14px 20px; padding: 0; font-size: 14px; line-height: 1.6;">
      <li>Open the install page on your phone (button below).</li>
      <li>Tap <strong>Sign in</strong> and use this email — ${escapeHtml(to)}. You don't have a password yet; enter anything the first time and we'll walk you through setting one. If you're new to us, you'll see a "Create account" button after trying — tap that.</li>
      <li>Tap your browser's share menu and "Add to Home Screen" so it launches like a real app.</li>
    </ol>
  ...
  const { html, text } = await renderEmailLayout({
    ...
    preheader: `Three steps to book bays, see your door code, and shop from your phone.`,
    bodyHtml,
    ctaButton: installUrl ? { label: "Get the app", url: installUrl } : null,
    footerText: tenantEmail?.email_footer_text,
  ...
  return sendEmail({
    to,
    subject: `The ${venueName} app is here — 3 steps to get set up`,
    html,
    text,
    fromOverride: tenantEmail?.email_from,
  });
...
export async function sendPaymentFailedEmail({
  tenantId, to, customerName, amount, description, portalUrl,
}) {
  const { tenantEmail, branding } = await loadTenantContext(tenantId);
  const venueName = branding?.app_name || tenantEmail?.name || "Ourlee";
