...
export async function sendLaunchEmail({
  ...
  const bodyHtml = `
    <p style="margin: 0 0 14px 0;">Hey ${escapeHtml(customerName || "there")},</p>
    <p style="margin: 0 0 14px 0;">We're excited to share that the new <strong>${escapeHtml(venueName)} app</strong> is ready. It's a new home for everything you do with us — booking a bay, your live door code, the pro shop, and your membership — all in one place on your phone.</p>
    <p style="margin: 0 0 12px 0;"><strong>Getting in takes under two minutes:</strong></p>
    <ol style="margin: 0 0 14px 20px; padding: 0; font-size: 14px; line-height: 1.65;">
      ...
      <li>Once you're signed in, tap your browser's share menu and <strong>Add to Home Screen</strong> so the app opens full-screen like a native one.</li>
    </ol>
  ...
  const { html, text } = await renderEmailLayout({
    ...
    footerText: tenantEmail?.email_footer_text,
    footerNote: "Reply any time — we'd love to help.",
  });
...
export async function sendCutoverAnnouncement({
  ...
  const bodyHtml = `
    ...
    <p style="margin: 0 0 14px 0;">On <strong>${escapeHtml(dateStr)}</strong>, the app becomes the way to book with us, and our old Skedda page will retire. You've got plenty of runway to get signed in whenever it's convenient.</p>
