...
// Build a header that surfaces the tenant's brand. Prefers the header
// logo; falls through to the welcome logo or the legacy single-slot
// logo if the header one isn't usable in email. Uses a stylized
// wordmark if nothing absolute is on file.
//
// Why the "usable in email" filter: email clients render `<img>` from
// ABSOLUTE URLs only. A relative path like "/blobs/logo.svg" works
// fine on the web but breaks in every inbox (Gmail / Apple Mail /
// Outlook all render it as an empty icon with a red X). We filter
// those out and fall through to the next available URL, ending at a
// wordmark rather than showing a broken image.
function isEmailSafeLogoUrl(u) {
  if (!u || typeof u !== "string") return false;
  // Absolute HTTPS (or HTTP) is what email clients will render.
  if (/^https?:\/\//i.test(u)) return true;
  // data: URLs work for small icons but most email filters strip
  // them. Treat as unsafe to be conservative.
  return false;
}

function pickHeaderLogoUrl(branding) {
  const candidates = [
  ...
  for (const c of candidates) {
    if (isEmailSafeLogoUrl(c)) return c;
  }
  return null;
}

// Email-safe logo size presets. Keyed on the tenant's
// branding.header_logo_size (s/m/l) which is already an admin-
// adjustable field (Settings → Branding slider). Values tuned to
// look intentional without overwhelming a narrow mobile inbox —
// "l" is the default since members consistently report the old
// 48px cap felt too small for a launch-email header.
//
// To globally bump: change these numbers. To change per tenant: flip
// header_logo_size in Settings → Branding.
const EMAIL_LOGO_SIZES = {
  s: { maxH: 48, maxW: 200 },
  m: { maxH: 72, maxW: 260 },
  l: { maxH: 96, maxW: 320 },
};
...
function renderHeader(branding, palette) {
  const logoUrl = pickHeaderLogoUrl(branding);
  const appName = branding?.app_name || "Ourlee";
  const size = EMAIL_LOGO_SIZES[branding?.header_logo_size] || EMAIL_LOGO_SIZES.l;
  if (logoUrl) {
    return `
    <tr>
      <td align="center" style="padding: 32px 24px 20px;">
        <img src="${escapeAttr(logoUrl)}" alt="${escapeAttr(appName)}" style="max-height: ${size.maxH}px; max-width: ${size.maxW}px; height: auto; width: auto; display: block;" />
      </td>
    </tr>`;
  }
  return `
    <tr>
      <td align="center" style="padding: 32px 24px 20px;">
        <div style="font-family: Georgia, 'Times New Roman', serif; font-size: 26px; font-weight: 700; letter-spacing: 1px; color: ${palette.primary};">
          ${escapeHtml(appName)}
        </div>
      </td>
    </tr>`;
